...
export default function App() {
  ...
  const fetchMembers = useCallback(() => {
    setLoading(true);
    // Render free tier can take ~15s to wake; show a warning after 4s
    const wakeTimer = setTimeout(() => setServerWaking(true), 4000);
    apiFetch('/api/members')
      .then((data) => {
        if (Array.isArray(data)) setMembers(data);
        else toast('Unexpected data format from server', 'warn');
      })
  ...
  useEffect(() => {
    ...
    socket.on('memberDeleted', ({ memberId }) => {
      setMembers((prev) =>
        prev.filter((raw) => norm(raw).member_id != memberId)
      );
    });
  ...
  const handleAddMember = async (e) => {
    ...
    try {
      await apiFetch('/api/new-member', {
        method: 'POST',
        body: JSON.stringify({ fullName: newName.trim(), carPlate: newPlate.trim().toUpperCase() }),
      });
  ...
  return (
    <>
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '40px 20px', fontFamily: "'DM Mono', monospace" }}>
        ...
        {serverWaking && (
          <div style={{
            ...
            padding: '12px 18px', marginBottom: 24, color: '#f59e0b', fontSize: 13,
          }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 11, color: '#10b981', letterSpacing: 3, marginBottom: 14, textTransform: 'uppercase' }}>
          ...
          <form onSubmit={handleAddMember} style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            ...
            <input
              type="text"
              placeholder="Plate (e.g. SBA1234A)"
              value={newPlate}
              onChange={(e) => setNewPlate(e.target.value.toUpperCase())}
              style={inputStyle({ flex: '1 1 140px', textTransform: 'uppercase' })}
            />
            <button
              type="submit"
              disabled={registering}
              style={{
                ...btnStyle('#10b981', '#0d0d1a'),
                flex: '0 0 auto', opacity: registering ? 0.6 : 1,
        ...
        <div style={{ marginBottom: 28, position: 'relative' }}>
          ...
          <input
            ...
            style={inputStyle({
              width: '100%', paddingLeft: 48, fontSize: 16,
            })}
          />
          {searchQuery && (
            <button
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: '60px 0', color: '#555' }}>
            <div style={{
              width: 36, height: 36, border: '3px solid #1e1e3f', borderTopColor: '#10b981',
                  ...
                  <div style={{ flex: '1 1 160px', minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', marginBottom: 6 }}>
                      ...
                      {member.car_plate && (
                        ...
                        }}>{member.car_plate.toUpperCase()}</span>
                      )}
